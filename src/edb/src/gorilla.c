 ...
 */
static inline double readFloat(Compressed_Iterator *iter, const u_int64_t *data) {
    binary_t xorValue;

    // Check if we can use the previous block info
    // meaning control bit number 2 is 1. i.e. control bits are 鈥�10鈥� (case b)
...
ChunkResult Compressed_ChunkIteratorGetNext(ChunkIter_t *abstractIter, double *sample) {
    ...
    // control bit 鈥�0鈥� (case a)
    *sample = Bins_bitoff(bins, iter->idx++) ? iter->prevValue.d : readFloat(iter, bins);
    iter->count++;
    return CR_OK;
}
