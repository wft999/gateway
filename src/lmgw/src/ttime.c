...
#include "../../lmgw/inc/ttime.h"

static int64_t parseFraction(const char* str, const char** end, int32_t timePrec);
static int32_t parseTimeWithTz(const char* timestr, int64_t* time, int32_t timePrec);
static int32_t parseLocaltime(const char* timestr, int64_t* time, int32_t timePrec);

const char *strnchr(const char *haystack, char needle, int32_t len, int skipquote) {
...
uint64_t taosGetTimestampMs() {
  ...
  return (uint64_t)systemTime.tv_sec * 1000LL + (uint64_t)systemTime.tv_usec / 1000;
}

uint64_t taosGetTimestampUs() {
  struct timeval systemTime;
  gettimeofday(&systemTime, NULL);
  return (uint64_t)systemTime.tv_sec * 1000000LL + (uint64_t)systemTime.tv_usec;
}
...
int32_t taosParseTime(const char* timestr, int64_t* time, int32_t len, int32_t timePrec) {
  ...
  if (strnchr(timestr, 'T', len, 0) != NULL) {
 ...
 */
int32_t parseTimeWithTz(const char* timestr, int64_t* time, int32_t timePrec) {
  int64_t factor = (timePrec == TSDB_TIME_PRECISION_MILLI) ? 1000 : 1000000;
  int64_t tzOffset = 0;

  struct tm tm = {0};
  const char*     str = strptime(timestr, "%Y-%m-%dT%H:%M:%S", &tm);
  if (str == NULL) {
    return -1;
  }
