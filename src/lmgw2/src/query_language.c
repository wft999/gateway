...
/*    if (chunkSizeBytes < 128) {
        RedisModule_ReplyWithError(
            ctx, "lmgw: CHUNK_SIZE value must be a multiple of 8 in the range [128 .. 1048576]");
        return false;
    }
...
/*    if (RMUtil_ArgIndex(arg_prefix, argv, argc) >= 0) {
        if (RMUtil_ParseArgsAfter(arg_prefix, argv, argc, "l", chunkSizeBytes) != REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse CHUNK_SIZE");
            return TSDB_ERROR;
        }
...
/*int ParseDuplicatePolicy(RedisModuleCtx *ctx,
    ...
    if (RMUtil_ArgIndex(arg_prefix, argv, argc) != -1) {
        if (RMUtil_ParseArgsAfter(arg_prefix, argv, argc, "s", &duplicationPolicyInput) !=
            REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse DUPLICATE_POLICY");
            return TSDB_ERROR;
        }

        DuplicatePolicy parsePolicy = RMStringLenDuplicationPolicyToEnum(duplicationPolicyInput);
        if (parsePolicy == DP_INVALID) {
            RedisModule_ReplyWithError(ctx, "lmgw: Unknown DUPLICATE_POLICY");
            return TSDB_ERROR;
        }
        *policy = parsePolicy;
        return TSDB_OK;
...
int parseCreateArgs(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, CreateCtx *cCtx) {
    ...
    if (parseLabelsFromArgs(argv, argc, &cCtx->labelsCount, &cCtx->labels) == REDISMODULE_ERR) {
        RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse LABELS");
        goto err_exit;
    }
...
/*    if (RMUtil_ArgIndex("RETENTION", argv, argc) > 0 &&
        RMUtil_ParseArgsAfter("RETENTION", argv, argc, "l", &cCtx->retentionTime) !=
            REDISMODULE_OK) {
        RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse RETENTION");
        goto err_exit;
    }
    ...
    if (encoding_location > 0) {
        ...
        if (strcasecmp(encoding, UNCOMPRESSED_ARG_STR) == 0) {
            ...
            *options |= SERIES_OPT_COMPRESSED_GORILLA;
            ...
            RedisModule_ReplyWithError(ctx, "lmgw: unknown ENCODING parameter");
            return TSDB_ERROR;
        }
    } else {
    ...
    *bucketTS = BucketStartTimestamp;
    ...
    if (bucketTS_offset > 0) {
        if (bucketTS_offset != AggregationOffset + 3 && bucketTS_offset != AggregationOffset + 4) {
            RedisModule_ReplyWithError(
                ctx,
                "lmgw: BUCKETTIMESTAMP flag should be the 3rd or 4th flag after AGGREGATION flag");
            return TSDB_ERROR;
        }
        ...
        if (strcasecmp(bucketTSStr, "start") == 0 || strcasecmp(bucketTSStr, "-") == 0) {
            ...
            *bucketTS = BucketMidTimestamp;
            ...
            RedisModule_ReplyWithError(ctx, "lmgw: unknown BUCKETTIMESTAMP parameter");
            return TSDB_ERROR;
        }
    }
    ...
    *alignmentTS = 0; // the default alignment is 0
    if (argc == 7) {
        ...
        if (parseTimestamp(argv[alignmentTS_offset], alignmentTS) != REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse alignTimestamp");
            return TSDB_ERROR;
        }

        if ((int64_t)(*alignmentTS) < 0) {
            RedisModule_ReplyWithError(ctx, "lmgw: alignTimestamp should be greater or equal to 0");
            return TSDB_ERROR;
        }
    }
...
int _parseAggregationArgs(RedisModuleCtx *ctx,
    ...
    if (offset > 0) {
        ...
        if (RMUtil_ParseArgs(argv, argc, offset + 1, "sl", &aggTypeStr, &temp_time_delta) !=
            REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse AGGREGATION");
            return TSDB_ERROR;
        }

        if (!aggTypeStr) {
            RedisModule_ReplyWithError(ctx, "lmgw: Unknown aggregation type");
            return TSDB_ERROR;
        }
        ...
        *agg_type = RMStringLenAggTypeToEnum(aggTypeStr);
        ...
        if (temp_time_delta <= 0) {
            RedisModule_ReplyWithError(ctx, "lmgw: bucketDuration must be greater than zero");
            return TSDB_ERROR;
        } else {
        ...
        if (empty) {
            ...
            if (empty_offset > 0) {
                if (empty_offset != offset + 3 && empty_offset != offset + 5) {
                    RedisModule_ReplyWithError(
                        ctx,
                        "lmgw: EMPTY flag should be the 3rd or 5th flag after AGGREGATION flag");
                    return TSDB_ERROR;
                }
                RedisModuleString *emptyStr = NULL;
                if (RMUtil_ParseArgs(argv, argc, empty_offset, "s", &emptyStr) != REDISMODULE_OK) {
                    RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse EMPTY flag");
                    return TSDB_ERROR;
                }
                *empty = false;
...
int parseAggregationArgs(RedisModuleCtx *ctx,
    ...
    if (result == TSDB_OK) {
        ...
        if (aggregationArgs.aggregationClass == NULL) {
            RedisModule_ReplyWithError(ctx, "lmgw: Failed to retrieve aggregation class");
            return TSDB_ERROR;
        }
        *out = aggregationArgs;
        return TSDB_OK;
...
static int parseCountArgument(RedisModuleCtx *ctx,
    ...
    if (offset > 0) {
        ...
        if (offset + 1 == argc) {
            RedisModule_ReplyWithError(ctx, "lmgw: COUNT argument is missing");
            return TSDB_ERROR;
        }
        if (RedisModule_StringToLongLong(argv[offset + 1], count) != REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse COUNT");
            return TSDB_ERROR;
        }
    }
    return TSDB_OK;
    ...
    if (align_location > 0) {
        ...
        if (strcasecmp(aligment, "start") == 0 || strcasecmp(aligment, "-") == 0) {
            ...
            *alignment = TimestampAlignment;
            ...
            RedisModule_ReplyWithError(ctx, "lmgw: unknown ALIGN parameter");
            return TSDB_ERROR;
        }
    }
    return TSDB_OK;
...
static int parseFilterByValueArgument(RedisModuleCtx *ctx,
    ...
    if (offset > 0) {
        if (offset + 2 >= argc) {
            RedisModule_ReplyWithError(ctx, "lmgw: FILTER_BY_VALUE one or more arguments are missing");
            return TSDB_ERROR;
        }

        if (RedisModule_StringToDouble(argv[offset + 1], &args->min) != REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse MIN");
            return TSDB_ERROR;
        }

        if (RedisModule_StringToDouble(argv[offset + 2], &args->max) != REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: Couldn't parse MAX");
            return TSDB_ERROR;
        }
        args->hasValue = true;
    }
...
static int parseFilterByTimestamp(RedisModuleCtx *ctx,
    ...
    if (offset > 0) {
        if (offset + 1 == argc) {
            RedisModule_ReplyWithError(ctx, "lmgw: FILTER_BY_TS one or more arguments are missing");
            return TSDB_ERROR;
        }
...
int parseRangeArguments(RedisModuleCtx *ctx,
    ...
    if (strcmp(start, "-") == 0) {
        ...
        if (parseTimestamp(argv[start_index], &args.startTimestamp) != REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: wrong fromTimestamp");
            return REDISMODULE_ERR;
        }
    }
    ...
    if (strcmp(end, "+") == 0) {
        ...
        if (parseTimestamp(argv[start_index + 1], &args.endTimestamp) != REDISMODULE_OK) {
            RedisModule_ReplyWithError(ctx, "lmgw: wrong toTimestamp");
            return REDISMODULE_ERR;
        }
    }
    ...
    if (args.alignment != DefaultAlignment) {
        if (args.aggregationArgs.aggregationClass == NULL) {
            RedisModule_ReplyWithError(ctx, "lmgw: ALIGN parameter can only be used with AGGREGATION");
            return TSDB_ERROR;
        }

        if (args.alignment == StartAlignment && startTimestampMin) {
            RedisModule_ReplyWithError(
                ctx, "lmgw: start alignment can only be used with explicit start timestamp");
            return TSDB_ERROR;
        }

        if (args.alignment == EndAlignment && endTimestampMax) {
            RedisModule_ReplyWithError(
                ctx, "lmgw: end alignment can only be used with explicit end timestamp");
            return TSDB_ERROR;
        }
    }
...
int parseMultiSeriesReduceArgs(RedisModuleCtx *ctx,
    ...
    if (agg_type == TS_AGG_FIRST || agg_type == TS_AGG_LAST || agg_type == TS_AGG_TWA ||
        ...
        RedisModule_ReplyWithError(ctx, "lmgw: Invalid reducer type");
        return TSDB_ERROR;
    }
    reducerArgs->aggregationClass = GetAggClass(agg_type);
    if (reducerArgs->aggregationClass == NULL) {
        RedisModule_ReplyWithError(ctx, "lmgw: Failed to retrieve reducer class");
        return TSDB_ERROR;
    }
...
int parseLabelQuery(RedisModuleCtx *ctx,
    ...
    *withLabels = RMUtil_ArgIndex("WITHLABELS", argv, argc) > 0;
    ...
    if (limit_location > 0 && *withLabels) {
        RedisModule_ReplyWithError(ctx, "lmgw: cannot accept WITHLABELS and SELECT_LABELS together");
        return REDISMODULE_ERR;
    }
    ...
    if (limit_location > 0) {
        ...
        for (int i = limit_location + 1; i < argc; i++) {
            ...
            if (count >= LIMIT_LABELS_SIZE) {
                RedisModule_ReplyWithError(ctx, "lmgw: reached max size for SELECT_LABELS");
                return REDISMODULE_ERR;
            }
        ...
        if (count == 0) {
            RedisModule_ReplyWithError(ctx, "lmgw: SELECT_LABELS should have at least 1 parameter");
            return REDISMODULE_ERR;
        }
...
int parseFilter(RedisModuleCtx *ctx,
    ...
    if (response == TSDB_ERROR) {
        ...
        RedisModule_ReplyWithError(ctx, "lmgw: failed parsing labels");
        return REDISMODULE_ERR;
    }

    if (CountPredicateType(queries, EQ) + CountPredicateType(queries, LIST_MATCH) == 0) {
        QueryPredicateList_Free(queries);
        RedisModule_ReplyWithError(ctx, "lmgw: please provide at least one matcher");
        return REDISMODULE_ERR;
    }
    *out = queries;
    return REDISMODULE_OK;
...
int parseMRangeCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, MRangeArgs *out) {
    ...
    if (filter_location == -1) {
        RedisModule_ReplyWithError(ctx, "lmgw: missing FILTER argument");
        return REDISMODULE_ERR;
    }
    ...
    if (groupby_location > 0 && groupby_location < filter_location) {
        RedisModule_ReplyWithError(ctx, "lmgw: GROUPBY should always come after filter");
        return REDISMODULE_ERR;
    }
    ...
    if (query_count == 0) {
        RedisModule_ReplyWithError(ctx, "lmgw: missing labels for filter argument");
        return REDISMODULE_ERR;
    }
